...
import { Search, User, Phone, Mail, Home, Calendar, ChevronRight, ArrowLeft, CheckCircle, XCircle, Clock, TruckIcon } from 'lucide-react';

interface MoveOutTrackerProps {
  onClose: () => void;
  initialFilter?: 'unapproved' | 'incomplete' | 'all';
  residentId?: string;
}

const MoveOutTracker: React.FC<MoveOutTrackerProps> = ({ 
  onClose, 
  initialFilter = 'all',
  residentId 
}) => {
  ...
  // Sample move-out data based on the spreadsheet
  const [moveOutResidents, setMoveOutResidents] = useState([
    {
      id: '410',
      name: 'Ruchira Mithisankara',
      unit: '410',
      moveOutDate: '2025-05-10',
      leaseEndDate: '2025-05-10',
      status: 'unapproved',
      residentTasks: {
        noticeToVacateSigned: false,
        moveOutInspectionComplete: false,
        keyReturn: false,
        finalUtilityReading: false,
    ...
    {
      id: '206',
      name: 'Sahasngaraj Rao',
      unit: '206',
      moveOutDate: '2025-04-15',
      leaseEndDate: '2025-04-15',
      status: 'incomplete',
      residentTasks: {
        noticeToVacateSigned: true,
        moveOutInspectionComplete: true,
        keyReturn: true,
        finalUtilityReading: true,
        forwardingAddress: true
      },
      operatorTasks: {
        removedAccess: false,
    ...
    {
      id: '407',
      name: 'Kayelene Catayong',
      unit: '407',
      moveOutDate: '2025-03-16',
      leaseEndDate: '2025-03-16',
      status: 'unapproved',
      residentTasks: {
        noticeToVacateSigned: false,
        moveOutInspectionComplete: false,
  ...
  const handleTaskToggle = (residentId: string, taskType: 'residentTasks' | 'operatorTasks', taskKey: string) => {
    ...
    setMoveOutResidents(prevResidents => 
  ...
  const getTaskDisplayName = (taskKey: string) => {
    const taskNames: { [key: string]: string } = {
      noticeToVacateSigned: 'Notice to Vacate Signed',
      moveOutInspectionComplete: 'Move Out Inspection Complete',
      keyReturn: 'Key Return',
      finalUtilityReading: 'Final Utility Reading',
      forwardingAddress: 'Forwarding Address',
      removedAccess: 'Removed Access',
      madeReady: 'Made Ready',
      finalInspection: 'Final Inspection',
      depositReturn: 'Deposit Return',
      unitTurnover: 'Unit Turnover'
    };
    return taskNames[taskKey] || taskKey.replace(/([A-Z])/g, ' $1').trim();
  };
      ...
      <div className="px-4 py-6 pb-24">
        <div className="mb-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Move-Out Tracker</h1>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Move-Out Overview</CardTitle>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{new Date(selectedResident.moveOutDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-600">Lease End Date</span>
                  <p className="font-medium">{new Date(selectedResident.leaseEndDate).toLocaleDateString()}</p>
                </div>
                ...
                {Object.entries(selectedResident.residentTasks).map(([key, completed]) => (
                  <div key={key} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <span>{getTaskDisplayName(key)}</span>
                    </div>
                    <Checkbox
    ...
    <div className="px-4 py-6 pb-24">
      <div className="mb-6">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Move-Out Tracker</h1>
            <p className="text-gray-600">Track resident move-out progress and tasks</p>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                      <div className="flex items-center space-x-2 text-sm text-gray-600">
                        ...
                        <span>Move-out: {new Date(resident.moveOutDate).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-sm text-gray-600">
      ...
      {filteredResidents.length === 0 && (
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No move-outs found</h3>
